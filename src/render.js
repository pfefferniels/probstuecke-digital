...
function generateSvg(params, allpages, callback, onFinish, onError) {
  fs.readFile(path.join(__dirname, '../data', params.nr, 'score.mei'), function(err, data) {
    if (err) {
      console.log(err);
...
function renderMusicExample(req, res) {
  ...
  fs.readFile(path.join(__dirname, '../data', req.query.nr, utils.preventDotDotSlash(req.query.filename)), function(err, data) {
    if (err) {
      console.log(err);
      res.status("404").end();
      return;
  ...
  traverse: function(tree) {
    ...
    for (let i=0; i<children.length; i++) {
      if (children[i].nodeName === "#text") {
        ...
        let contents = fs.readFileSync(path.join(__dirname, "../data", this.nr, target), 'utf8');
...
function downloadPDF(req, res) {
  ...
  if (req.query.exportFormat === "pdf") {
    ...
    generateSvg(req.query, true, function(svg) {
      ...
      fs.readFile(utils.getAnnotationFilename(req.query.nr, req.query.lang), function(err, data) {
        if (err) {
          console.log(err);
          res.status("404").end();
          return;
...
exports.renderMusicExample = renderMusicExample;
